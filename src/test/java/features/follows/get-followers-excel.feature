Feature: Get Followers Testing từ Excel - Sheet getFollowers
  ...
  Scenario: Chạy tất cả test cases get followers từ Excel
        ...
        if (testCase.expectedResult && testCase.expectedResult.trim() !== '') {
          try {
            ...
            if (expectedStatus == 200 || expectedStatus == 201) {
              if (expectedObj.status && actualResponse.status != expectedObj.status) {
                return {
                  status: 'FAIL',
                  reason: 'Status field mismatch in response body'
                };
              }
              if (expectedObj.data !== undefined && actualResponse.data === undefined) {
                return {
                  status: 'FAIL',
                  reason: 'Missing data field in success response'
                };
              }
            }
    ...
    * def runTest = 
      ...
      function(testCase, index) {
        ...
        // Gọi API get followers
        var response = karate.call('classpath:features/helpers/get-followers-single-call.feature', {
    ...
    * print '=== GET FOLLOWERS TEST SUMMARY ==='
    ...
    * print 'Hoàn thành tất cả test cases get followers. Đang ghi kết quả vào Excel...'
    
    # Ghi kết quả vào Excel
    * def writeSuccess = writeToExcel(excelFilePath, sheetName, testResults)
    * if (writeSuccess) karate.log('✅ Đã ghi kết quả get followers vào Excel thành công!')
    * if (!writeSuccess) karate.log('❌ Lỗi khi ghi kết quả get followers vào Excel!')
