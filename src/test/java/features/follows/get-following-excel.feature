Feature: Get Following Testing từ Excel - Sheet getFollowing
  ...
  Scenario: Chạy tất cả test cases get following từ Excel
        ...
        if (testCase.expectedResult && testCase.expectedResult.trim() !== '') {
          try {
            ...
            if (expectedStatus == 200 || expectedStatus == 201) {
              ...
              if (expectedObj.statusCode && actualResponse.statusCode != expectedObj.statusCode) {
                return {
                  status: 'FAIL',
                  reason: 'StatusCode mismatch in response body'
                };
              }
            }
    ...
    * def runTest = 
      ...
      function(testCase, index) {
        ...
        // Gọi API get following
        var response = karate.call('classpath:features/helpers/get-following-single-call.feature', {
    ...
    * print '=== GET FOLLOWING TEST SUMMARY ==='
    ...
    * print 'Hoàn thành tất cả test cases get following. Đang ghi kết quả vào Excel...'
    
    # Ghi kết quả vào Excel
    * def writeSuccess = writeToExcel(excelFilePath, sheetName, testResults)
    * if (writeSuccess) karate.log('✅ Đã ghi kết quả get following vào Excel thành công!')
    * if (!writeSuccess) karate.log('❌ Lỗi khi ghi kết quả get following vào Excel!')
