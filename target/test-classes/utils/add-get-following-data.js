function(filePath, sheetName) {
    ...
    try {
        ...
        // Dữ liệu test cases cho getFollowing
        var testCases = [
            {
                bearerToken: '', // TC1: Sử dụng global token
                expectedStatus: '200',
                expectedResult: '{"message":"Danh sách đang theo dõi","statusCode":200,"status":"success"}',
                testDescription: 'TC1: Lấy danh sách following thành công với token hợp lệ'
            },
            {
                bearerToken: 'NO_TOKEN',
                expectedStatus: '401',
                expectedResult: '{"error": "Unauthorized"}',
                testDescription: 'TC2: Lấy danh sách following thất bại - Không truyền bearer token'
            },
            {
                bearerToken: 'invalid_token_12345',
                expectedStatus: '401',
                expectedResult: '{"error": "Invalid or expired token"}',
                testDescription: 'TC3: Lấy danh sách following thất bại - Bearer token không hợp lệ'
            }
        ];
        ...
        karate.log('Đã thêm ' + testCases.length + ' test cases getFollowing vào Excel');
        return true;

    } catch (e) {
        karate.log('Lỗi khi thêm dữ liệu getFollowing:', e);
        return false;
    }
} 