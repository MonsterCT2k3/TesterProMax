Feature: Logout Testing từ Excel - Sheet logout
  ...
  Scenario: Chạy tất cả test cases logout từ Excel
    * def runTest = 
      """
    ...
    * print 'Hoàn thành tất cả test cases logout. Đang ghi kết quả vào Excel...'
    
    * print 'Ghi kết quả vào Excel...'
    * def writeSuccess = writeToExcel(excelFilePath, sheetName, testResults)
    * if (writeSuccess) karate.log('Đã ghi kết quả logout vào Excel thành công!')
    * if (!writeSuccess) karate.log('Lỗi khi ghi kết quả logout vào Excel!') 