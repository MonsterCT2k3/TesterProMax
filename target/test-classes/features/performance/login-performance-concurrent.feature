Feature: Login API Performance Test - 100 Concurrent Calls

  Background:
    * url 'http://localhost:3000'

  Scenario Outline: Single Login Call per Scenario
    * def currentUser = { email: '<email>', password: '<password>' }
    * print '*** CONCURRENT API CALL for user:', currentUser.email, '***'
    
    Given url 'http://localhost:3000/auth/login'
    And request currentUser  
    When method POST
    Then status 200
    And print 'Concurrent call completed, status:', responseStatus
